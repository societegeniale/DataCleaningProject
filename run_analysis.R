#The Samsung data ("UCI HAR Dataset" directory)
# must be in the same directory as this script
# : load and merge dataset, make its variable names more readable,
# calculate the mean of the each variable by activity and subject and write the result
# into a meanMotionRecords.txt
...
mergeDataSet <- function() {
  ...
  variableNames <- read.table("UCI HAR Dataset\\features.txt", col.names=c("id","variable"))
  ...
  trainRecords <- read.table("UCI HAR Dataset\\train\\X_train.txt", col.names=variableNames$variable)
  trainRecords <- trainRecords[,grep("(mean|std)\\.\\.",names(trainRecords))]
  testRecords <- read.table("UCI HAR Dataset\\test\\X_test.txt", col.names=variableNames$variable)
  testRecords <- testRecords[,grep("(mean|std)\\.\\.",names(testRecords))]
  
  # add activity names to the train and test records
  trainActivities <- read.table("UCI HAR Dataset\\train\\y_train.txt", col.names=c("id"))
  ...
  testActivities <- read.table("UCI HAR Dataset\\test\\y_test.txt", col.names=c("id"))
  ...
  testSubjects <- read.table("UCI HAR Dataset\\test\\subject_test.txt", col.names=c("subject"))
  testRecords <- mutate(testRecords, subject=testSubjects$subject)
  trainSubjects <- read.table("UCI HAR Dataset\\train\\subject_train.txt", col.names=c("subject"))
